package es.uc3m.inf.kr.oslc.km.neo4j;
...
public class Neo4jMapDBBatchHandler implements RDFHandler {
	...
	private HTreeMap<Object, Long> tmpIndex;

	private static final String URI_PROPERTY = "__URI__";

	public Neo4jMapDBBatchHandler(BatchInserter db2, int indexCache, int timeout) {
		this.db = db2;
		...
		DB db = DBMaker.newDirectMemoryDB().deleteFilesAfterClose().writeAheadLogDisable().closeOnJvmShutdown().make();
		tmpIndex = db.createHashMap("entities", false, SerializerBase.BASIC_SERIALIZER, SerializerBase.LONG_SERIALIZER);
	}
	...
	public void handleStatement(Statement st) {
		try {
			...
			if (predicate.stringValue().equals("http://www.w3.org/1999/02/22-rdf-syntax-ns#type")) {
				...
				nodeProps.put(predicate.stringValue(), value);
				...
				RelationshipType relType = DynamicRelationshipType.withName(predicate.stringValue());
				boolean hit = false;
